 ...
 *   bun --env-file=.env scripts/bootstrap-super-admin.ts --stage production user@example.com
 *   bun --env-file=.env scripts/bootstrap-super-admin.ts --stage dev user@example.com
 */
...
const dbId = STAGE_DB[stageFlag];

if (!accountId || !token) {
  console.error("Missing CLOUDFLARE_ACCOUNT_ID or CLOUDFLARE_API_TOKEN in packages/infra/.env");
  process.exit(1);
}
if (!dbId) {
