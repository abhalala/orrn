...
const PLATFORM_NAV: readonly { href: string; label: string; icon: ReactNode }[] = [
  { href: `${appUrls.staff}/admin`, label: "Console", icon: <Shield size={16} /> },
  { href: `${appUrls.staff}/admin/companies`, label: "Companies", icon: <Building2 size={16} /> },
  { href: `${appUrls.staff}/admin/waitlist`, label: "Waitlist", icon: <ClipboardList size={16} /> },
];
            ...
            <SidebarSection label="Platform">
              {PLATFORM_NAV.map((item) => (
                <a key={item.href} href={item.href} className="no-underline">
                  <SidebarItem active={false} icon={item.icon}>
                    {item.label}
                  </SidebarItem>
                </a>
              ))}
