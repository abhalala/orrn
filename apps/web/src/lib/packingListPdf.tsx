...
// Document component
// ---------------------------------------------------------------------------
function PackingListDoc({ pl, code }: { pl: PLSnapshot; code: string }) {
    ...
    <Document>
      <Page size="A4" style={s.page}>
        ...
        <View style={s.tableHeader}>
          ...
          <Text style={[s.th, s.w6, { textAlign: "right" }]}>Len (m)</Text>
        </View>
        {pl.items.map((item, i) => (
          <View key={i} style={[s.tableRow, { backgroundColor: i % 2 === 0 ? "#fff" : "#fafafa" }]}>
            ...
            <Text style={[s.td, s.w6]}>{(item.lengthMm / 1000).toFixed(3)}</Text>
          </View>
        ))}
        ...
        <View style={s.totalsRow}>
          ...
          <View>
            ...
            <Text style={s.totalsValue}>{pl.totals.totalLengthM} m</Text>
          </View>
        </View>
...
// Download helper
// ---------------------------------------------------------------------------
export async function downloadPackingListPdf(pl: PLSnapshot, code: string) {
  const blob = await pdf(<PackingListDoc pl={pl} code={code} />).toBlob();
