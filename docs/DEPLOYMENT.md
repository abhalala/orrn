...
ORRN runs on **Cloudflare Workers** (API) and **Workers static assets** (Vite web), with **D1** for SQLite. Infrastructure is defined in `packages/infra/alchemy.run.ts` and deployed with [Alchemy](https://alchemy.run).

## Domain layout (three web Workers + API)

Each hostname is a **separate Vite Worker** (`packages/infra/alchemy.run.ts`).


| Host          | Zone       | Worker   | Role                                    |
| ------------- | ---------- | -------- | --------------------------------------- |
| `orrn.in`     | `orrn.in`  | `web`    | Marketing, tenant login, waitlist       |
| `erp.orrn.in` | `orrn.in`  | `erp`    | Tenant ERP (unauthenticated → marketing login) |
| `orrn.app`    | `orrn.app` | `admin`  | Staff console (email/password only)     |
...
| Host               | Worker   | Role        |
| ------------------ | -------- | ----------- |
| `dev.orrn.app`     | `web`    | Marketing   |
| `erp.dev.orrn.app` | `erp`    | ERP         |
| `staff.dev.orrn.app` | `admin` | Staff console |
| `api.dev.orrn.app` | `server` | API         |

Production deploy sets `VITE_MARKETING_URL`, `VITE_ERP_URL`, and `VITE_STAFF_URL` on all three web Workers (see `.github/workflows/deploy-prod.yml`).

## Cloudflare prerequisites

1. Both zones `**orrn.in`** and `**orrn.app**` on the same Cloudflare account.
...
| `CLOUDFLARE_ZONE_ID_APP`                                        | Zone ID for `orrn.app`                                   |
...
**Verify token before deploy** (from `packages/infra`):
...
export CLOUDFLARE_ZONE_ID_APP=...      # orrn.app
...
- `orrn.in` → web Worker
- `erp` → web Worker (same deployment, extra hostname)
- `api` → server Worker

`**orrn.app` zone**

- `orrn.app` → web Worker (staff UI)
...
- Session cookies: `Domain=.orrn.in` (works for `api.orrn.in`, `erp.orrn.in`, `orrn.in`)
- `orrn.app` UI calls `api.orrn.in` with `credentials: include`; cookies are sent to the API host
- `CORS_ALLOWED_ORIGINS` includes `https://orrn.app` explicitly (different registrable domain from `orrn.in`)

Waitlist / magic-link invites use `WEB_ERP_URL` (`https://erp.orrn.in` in prod).
...
Staff logins are **not** self-service on `orrn.app`. Grant `super_admin` to an **existing** Better Auth user:

```bash
cd packages/infra
# After the user has signed up (marketing or ERP), e.g. on https://orrn.in/login
bun run bootstrap-super-admin --stage production you@company.com
# Dev:
bun run bootstrap-super-admin --stage dev you@company.com
```

Then sign in at `https://orrn.app` and open **Admin → Staff** to create additional staff with roles (`admin`, `support`).

## Verify after deploy

```bash
curl -sI https://orrn.in | head -5
curl -sI https://erp.orrn.in | head -5
curl -sI https://orrn.app | head -5
curl -sI https://api.orrn.in/trpc/auth.me | head -5
```

Sign in on `https://orrn.in/login`, confirm redirect to `https://erp.orrn.in/dashboard`. Platform admin: `https://orrn.app/admin`.